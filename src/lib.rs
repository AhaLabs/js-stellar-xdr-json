...
#[wasm_bindgen]
pub fn decode(type_variant: String, xdr_base64: String) -> Result<JsValue, String> {
    ...
    let json = serde_wasm_bindgen::to_value(&value).map_err(|e| format!("{e}"))?;
    Ok(json)
}
...
#[wasm_bindgen]
pub fn encode(type_variant: String, json: JsValue) -> Result<String, String> {
    let type_variant = type_variant.parse().map_err(|e| format!("{e}"))?;
    // TODO: Return a native JS value.
    // let t: Type = serde_wasm_bindgen::from_value(js).map_err(|e| format!("{e}"))?;
    let t = Type::deserialize_json_wasm(type_variant, json).map_err(|e| format!("{e}"))?;
    let b64 = t
